...
__version__ = u'2.4.1+p1'

release_notes = """
2010-09-17  RaphaelBossek
* Release v2.4.1+p1
* FIX: Missing pictures for ACL protected pages with MoinMoin
...
if is_moinmoin_version_eqless(1,8):
    ...
    from MoinMoin.web.session import get_cookie_name
...
""" % (wikiutil.escape(self.values['extra-titledocnumber']), wikiutil.escape(self.values['extra-titleauthor']), wikiutil.escape(self.values['extra-titlecopyright']),) \
    ...
    def html2pdf(self, html):
        ...
        htmldocopts = [self.default_values['htmldoc_cmd'], u'--no-duplex'] + self.default_values['htmldoc_options']

        for cookie in self.get_session_cookies():
    ...
    def get_session_cookies(self):
        """It's important to set the right cookies to get access to ACL protected pages, especially their attachments
        because the HTML code itself is already there if we call HTMLDOC. HTMLDOC have to download the attachments
        itself (in his own socket connection) using theses cookies.
        """
        result=["MOIN_ID=" + self.request.user.id]
        cookie = None
        # With MoinMoin 1.9 the usage of cookies for authentication changed.
        # Please refer to MoinMoin/web/session.py:get_cookie_name() function.
        if is_moinmoin_version_eqhigher(1,9):
            cookie_name = get_cookie_name(self.request, name=self.request.cfg.cookie_name, usage=self.request.cfg.session_service.cookie_usage)

        # For MoinMoin 1.6 and newer MOIN_SESSION is mandatory to get access to ACL protected attachments.
        # The ongoing session id will be used for HTMLDOC request.
        elif is_moinmoin_version_eqhigher(1,6):
            cookie_name = "MOIN_SESSION"

        # For MoinMoin 1.5 the MOIN_SESSION* cookie is not required.
        else:
            cookie_name = None

        if cookie_name:
            if hasattr(self.request, 'cookie') and self.request.cookie.get(cookie_name, False):
                cookie = cookie_name + "=" + self.request.cookie.get(cookie_name).value
            # Since MoinMoin 1.9 it's called "cookies".
            elif hasattr(self.request, 'cookies') and self.request.cookies.get(cookie_name, False):
                cookie = cookie_name + "=" + self.request.cookies.get(cookie_name)
            # Backward compatibility before MoinMoin 1.7
                ...
                cookies = ';'
                if hasattr(self.request, 'headers') and hasattr(self.request.headers, 'cookie'):
                    cookies = self.request.headers.cookie
                # Since MoinMoin 1.6 it's called 'saved_cookies'
                elif hasattr(self.request, 'saved_cookie'):
                    cookies = self.request.saved_cookie

                for cookie in cookies.split(';'):
                    # Remove trailing while spaces.
                    cookie = cookie.strip()
                    # All kinds of MOIN_SESSION.* willbe accepted
